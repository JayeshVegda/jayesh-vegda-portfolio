  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 pt-8 pb-6 px-6 h-full flex flex-col gap-5">
          {/* Image with 16:9 ratio and gradient overlay */}
          <div className="relative w-full aspect-video rounded-lg overflow-hidden -mx-2 mt-2">
            <Image
          ...
          <div className="flex-1 space-y-3">
            {/* Title */}
            <h5 
              className="text-xl font-semibold tracking-tight text-foreground/90 dark:text-white/90"
              style={{
            ...
            <p 
              className="line-clamp-2 font-normal text-foreground/70 dark:text-white/70 leading-relaxed text-sm"
              style={{
                fontFamily: "var(--font-sans), -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif",
              }}
            >
            ...
            <div className="flex gap-1.5 flex-wrap">
              {techStackLabels.map((label, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-light border-[0.5px] border-black/10 dark:border-white/10 bg-white/40 dark:bg-black/10 text-foreground/60 dark:text-white/60 backdrop-blur-sm"
                >
                  {label}
                </span>
              ))}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              {project.githubLink ? (
                ...
                >
                  <Icons.gitHub className="w-4 h-4 text-foreground/60 dark:text-white/60 hover:text-foreground/90 dark:hover:text-white/90 transition-colors" />
                </Link>
              ) : (
                <div className="p-1.5 rounded-lg opacity-40 cursor-not-allowed">
                  <Icons.gitHub className="w-4 h-4 text-foreground/30 dark:text-white/30" />
                </div>
              )}
              ...
              {project.websiteLink ? (
                <Link 
                  ...
                  className="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                >
                  <Icons.externalLink className="w-4 h-4 text-foreground/60 dark:text-white/60 hover:text-foreground/90 dark:hover:text-white/90 transition-colors" />
                </Link>
              ) : (
                <div className="p-1.5 rounded-lg opacity-40 cursor-not-allowed">
                  <Icons.externalLink className="w-4 h-4 text-foreground/30 dark:text-white/30" />
                </div>
              )}
            </div>
            ...
            <Link 
              ...
              className="inline-flex items-center gap-1.5 text-xs font-medium text-foreground/70 dark:text-white/70 hover:text-foreground/90 dark:hover:text-white/90 transition-colors group/explore"
            >
              <span>Explore Project</span>
              <Icons.chevronRight className="w-3.5 h-3.5 group-hover/explore:translate-x-0.5 transition-transform" />
            </Link>
          </div>
        </div>
      </motion.div>
